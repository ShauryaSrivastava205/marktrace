...
import { tierMove } from "@/lib/verify"
...
interface ConfidenceBadgeProps {
  /** The tier the diagnosis carried before this probe, or null if unknown. */
  previous: RootGapConfidence | null
...
export function ConfidenceBadge({ previous, current, delay = 0 }: ConfidenceBadgeProps) {
  ...
  // With no earlier tier there is no move to describe, so the badge shows the
  // tier this probe produced and says nothing about a change.
  const move = previous ? tierMove(previous, current) : null
  const upgraded = move === "upgraded"
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2.5">
        {previous && (
          <>
            ...
            >
              <span
                aria-hidden="true"
                className={cn("size-1.5 rounded-full", TIER_STYLE[previous].dot)}
              />
              {previous}
        ...
        >
          {move === "upgraded"
            ? "upgraded by this probe"
            : move === "lowered"
              ? "lowered by this probe"
              : move === "unchanged"
                ? "unchanged by this probe"
                : "from this probe"}
        </motion.span>
      </div>
    </div>
